...
void find_file_and_copy_to_temp(void)
{
    ...
    snprintf(command, 1000, "find %s -name %s -type f >> %s 2>&1",current_dir,current_file,temp_file);  // function which concatenates command into one char*
...
int reserve_block (struct Array * array)
{
    ...
    if (1 != fread(array->array[index], filesize, 1, f))  //getting entire file to array; fread (to, sizeofelement, no_of_elements,fromwhere))
    {
        printf("Error in reading file\n");
        fclose(f);
...
{// this fun frees char** array and temp file, current directory and current file
    if (temp_file != NULL) free(temp_file);
    if (current_file != NULL) free(current_file);
    if (current_dir != NULL) free(current_dir);
